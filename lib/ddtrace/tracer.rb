...
module Datadog
  ...
  class Tracer
    ...
    def build_span(name, options = {})
      ...
      service = options[:service] || (parent && parent.service) || default_service
      tags = @tags.any? && options[:tags] ? @tags.merge(options[:tags]) : options[:tags] || @tags.dup

      # Build a new span operation
      span = Datadog::SpanOperation.build(
        name,
        options.merge(
          tracer: self,
          context: context,
  ...
  class SpanOperation
    ...
    def self.build(span_name, options = {})
      ...
      span = Span.new(
        span_name,
        options # Should this filter options?
      )

      # Wrap it in an operation
      new(span, { tracer: Datadog.tracer }.merge(options))
    end
    ...
    def initialize(span, options = {})
      ...
      # Add it to the context, if provided.
      @context.add_span(self) if @context

      if options[:child_of].nil?
